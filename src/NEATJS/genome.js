...
import BrainNode from './node.js'

let activationsNames = ["Sigmoid", "Identity", "Step", "Tanh", "ReLu"]; //Used in the svg drawing
...
class Genome {
	constructor(inp, out, id, offSpring = false) {
		...
		if(!offSpring) { //This is not an offspring genome generate a fullyConnected net
			for (let i = 0; i < this.inputs; i++) {
				this.nodes.push(new BrainNode(this.nextNode, 0));
				this.nextNode++;
			}

			for (let i = 0; i < this.outputs; i++) {
				let node = new BrainNode(this.nextNode, 1, true);
	...
	addNode() { //Add a node to the network
		...
		//Create the new node
		let newNode = new BrainNode(this.nextNode, pickedConnection.fromNode.layer + 1);
